...
/**
 * Plugin Name: ZNPB Breadcrumbs Element
 * Plugin URI: http://hogash.com
 * Description: This plugin will generate a breadcrumb element for Kallyas Page builder.
 * Version: 1.0.1
...
class ZnPbBreadcrumbsElement{
	...
	function __construct(){
		...
		load_plugin_textdomain('znpb-breadcrumbs-element', false, basename( dirname( __FILE__ ) ) . '/languages' );
	}
	...
	function show_admin_notice(){
		...
		$buy_link = sprintf( '<a class="button button button-primary" href="%1$s" target="_blank">%2$s</a>', $buy_kallyas_url, __( 'Get Kallyas theme from here', 'znpb-breadcrumbs-element' ) );
		$message = __( 'Kallyas theme is not installed! Breadcrumbs Element only works with Kallyas theme.', 'znpb-breadcrumbs-element' );
...
ZnPbBreadcrumbsElement::get_instance();