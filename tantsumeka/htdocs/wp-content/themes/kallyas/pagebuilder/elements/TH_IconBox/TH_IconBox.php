...
class TH_IconBox extends ZnElements
{
	...
	function css(){
		...
		if(!empty($title_styles)){
			$css .= '.'.$uid.' .kl-iconbox__inner .kl-iconbox__title {'.$title_styles.'} ';
		}
		...
		if( is_array($desc_typo) && !empty($desc_typo) ){
			...
			$css .= '.'.$uid.' .kl-iconbox__inner .kl-iconbox__desc {'.$desc_styles.'} ';
		}
		...
		if( $this->opt('ibx_type', 'icon') == 'icon' ){
			...
			$css .= '.'.$uid.' .kl-iconbox__inner .kl-iconbox__icon {color:'.$ibx_icon_color.'; '.$cc_stroke_box_sh.'} ';
			$css .= '.'.$uid.':hover .kl-iconbox__inner .kl-iconbox__icon {color:'.$ibx_icon_color_hover.'; '.$cc_stroke_box_sh_hov.'} ';
		}

		// If has a shape behind
		if( $ibx_shape != '' && $ibx_shape != 'sh-circle-stroke' ){
			$css .= '.'.$uid.'.kl-iconbox--sh .kl-iconbox__inner .kl-iconbox__icon {background-color:'.$this->opt('ibx_shape_color', '#dfdfdf' ).'} ';
			$css .= '.'.$uid.'.kl-iconbox--sh .kl-iconbox__inner .kl-iconbox__icon:after, .'.$uid.'.kl-iconbox--sh:hover .kl-iconbox__inner img.kl-iconbox__icon  {background-color:'.$this->opt('ibx_shape_color_hover', '#cd2122' ).'} ';
		}

		// Icon sizes
		$icon_size = $this->opt('ibx_size','42');
		if( $icon_size != '42' && $icon_type == 'icon'){
			$css .= ".{$uid} .kl-iconbox__inner span.kl-iconbox__icon { font-size: {$icon_size}px }";
		}

		// Image size
		$img_size = $this->opt('ibx_imgwidth','100');
		if( $img_size != '100' && $icon_type == 'img'){
			$css .= ".{$uid} .kl-iconbox__inner img.kl-iconbox__icon { width: {$img_size}px }";
		}

		// Icon Shaped Padding
		$ibx_shaped_padding = $this->opt('ibx_shaped_padding','22');
		if( $ibx_shaped_padding != '22' && $ibx_shape != '' ){
			$css .= ".{$uid} .kl-iconbox__inner .kl-iconbox__icon { padding: {$ibx_shaped_padding}px }";
		}

		// Icon Opacity
		$ibx_opacity = $this->opt('ibx_opacity','100');
		if( $ibx_opacity != '100' && $ibx_opacity != '' ){
			$css .= '.'.$uid.' .kl-iconbox__inner .kl-iconbox__icon {opacity: '.($ibx_opacity/100).'; }';
		}

		// Add floated side margin
		$floated_side_distance = $this->opt('floated_side_distance','20');
		$ibx_floated = $this->opt('ibx_floated','');
		if( $ibx_floated != '' &&  $floated_side_distance != '20'  ){
			$css .= '@media screen and (min-width: 481px){.'.$uid.'.kl-iconbox--'.$ibx_floated.' .kl-iconbox__inner .kl-iconbox__icon-wrapper {margin-'.substr($ibx_floated, 1).':'.$floated_side_distance.'px}}';
		}
	...
	function element()
	{
		...
		if( !empty($ibstg_point_stage) ){
			...
			//#! Include measurement
            $unit = $this->opt( 'point_measure_unit' );
            if( ! empty($unit) && in_array($unit,array('pixel', 'percent'))){
				$unit = ( ('pixel' == $unit) ? 'px' : '%' );
                $points .= 'data-unit="'.esc_attr($unit).'"';
            }

			//  add title tooltip
			if( $this->opt('ibstg_point_title','') ){
				$title = $this->opt('ibstg_point_title');
				$points .= ' data-pointtitle="'.htmlspecialchars($title, ENT_QUOTES ).'"';
			}
			//  add point number
		...
		$attributes = zn_get_element_attributes($options, $uid);
		...
		return array (
			...
			'styling' => array(
				...
				'options' => array(
					...
					array (
						"name"        => __( "Floated-Icon Side Distance", 'zn_framework' ),
						"description" => __( "Select the distance between the side floated icon and text content.", 'zn_framework' ),
						"id"          => "floated_side_distance",
						"std"         => "20",
						"type"        => "slider",
						"helpers"     => array (
							"step" => "1",
						...
						"dependency"  => array(
							...
							array( 'element' => 'ibx_floated' , 'value'=> array('fleft', 'fright') )
						),
						'live'        => array(
							'multiple' => array(
								array(
									'type'      => 'css',
									'css_class' => '.'.$uid.'.kl-iconbox--fleft .kl-iconbox__icon-wrapper ',
									'css_rule'  => 'margin-right',
									'unit'      => 'px'
								),
								array(
									'type'      => 'css',
									'css_class' => '.'.$uid.'.kl-iconbox--fright .kl-iconbox__icon-wrapper ',
				   ...
				   array (
					...
					array (
						...
						'live' => array(
						   'type'        => 'css',
						   'css_class' => '.'.$uid.'.kl-iconbox--sh .kl-iconbox__icon',
					...
					array (
						...
						'live' => array(
							'type'        => 'class',
							'css_class' => '.'.$uid .' .kl-iconbox__icon',
			...
			'help' => znpb_get_helptab( array(
				'video'   => sprintf( '%s', esc_url('https://my.hogash.com/video_category/kallyas-wordpress-theme/#F1ttWpjkKqQ') ),
				'docs'    => sprintf( '%s', esc_url('https://my.hogash.com/documentation/icon-box/') ),
				'copy'    => $uid,
				'general' => true,
			)),
