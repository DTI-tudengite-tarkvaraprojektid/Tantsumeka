...
class TH_NewsletterBox extends ZnElements
{
	...
	function element()
	{
        ...
        <div class="elm-nlbox <?php echo implode( ' ', $elm_classes ); ?>" <?php echo $attributes; ?>>
			...
			if ( !empty ( $nl_id ) ) {
            ?>
                <form method="post" class="elm-nlbox__form js-mcForm clearfix" data-url="<?php echo trailingslashit( home_url() ) ?>" name="newsletter_form">
                    <input type="text" name="mc_email" class="elm-nlbox__input nl-email form-control js-mcForm-email <?php echo $corner_radius; ?>" value="" placeholder="<?php echo $this->opt( 'nb_em_pl' ); ?>" required="required"/>
                    <button type="submit" name="submit" class="elm-nlbox__submit <?php echo $corner_radius; ?>">
						<?php
					...
					<input type="hidden" name="mailchimp_list" class="nl-lid" value="<?php echo $nl_id; ?>"/>
