...
<div class="kl-slideshow static-content__slideshow <?php echo $style; ?> <?php echo $this->data['uid']; ?> <?php echo $bm_class ?> <?php echo zn_get_element_classes($this->data['options']); ?>">
	...
	<div class="kl-slideshow-inner static-content__wrapper <?php echo ( $this->opt('sc_fullscreen', '0') ? 'static-content--fullscreen' : '' ); ?> <?php echo ( (int)$this->opt('ww_height') ? 'static-content--height':'' ) ?>">
		...
		<div class="static-content__inner container">
			...
			<div class="kl-slideshow-safepadding sc__container <?php echo $obj_parallax__class; ?>" <?php echo $obj_parallax__attribs; ?>>
					...
					<div class="row">
						<div class="col-sm-10 col-sm-offset-1 col-md-7 col-md-offset-0">
							...
							<div class="ud_counter kl-counter kl-font-alt">
								...
								<?php echo '<span class="till_lauch kl-counter-launch"><img class="kl-counter-launch-img" src="'.THEME_BASE_URI.'/images/rocket.png" alt="'.esc_html(__( 'Launch Date', 'zn_framework' )).'"></span>'; ?>
							</div><!-- end counter -->

							<?php
							if ( !empty ( $options['sc_ec_mlid'] ) ) {
								echo '<div class="mail_when_ready kl-newsletter-wrapper">';
								echo	'		<form method="post" class="js-mcForm newsletter_subscribe newsletter-signup kl-newsletter clearfix" data-url="'.home_url('/').'" name="newsletter_form">';
								echo	'			<input type="text" name="mc_email" class="nl-email form-control kl-newsletter-field js-mcForm-email" value="" placeholder="'.esc_attr(__("your.address@email.com",'zn_framework')).'" />';
								echo	'			<input type="hidden" name="mailchimp_list" class="nl-lid" value="'.esc_attr($options['sc_ec_mlid']).'" />';
								...
								echo	'			<input type="submit" name="submit" class="kl-newsletter-submit kl-font-alt btn btn-fullcolor" value="'.esc_html(__("JOIN US",'zn_framework')).'" />';
	...
	function options()
	{
		...
		$options = array(
			...
			'help' => znpb_get_helptab( array(
				'video'   => sprintf( '%s', esc_url('https://my.hogash.com/video_category/kallyas-wordpress-theme/#eH7M2AWz_OI') ),
				'docs'    => sprintf( '%s', esc_url('https://my.hogash.com/documentation/static-content-event-countdown/') ),
				'copy'    => $uid,
				'general' => true,
			)),
