	...
	function addAdminNotices(){
		...
		?>
			<div class="notice notice-error">
				...
				<p><a class="button button button-primary" href="<?php echo admin_url( ZNHGTFW()->getComponent('utility')->get_options_page_base_url() . '?page=zn-about#zn-about-tab-addons-dashboard' ); ?>"><?php _e( 'Install and activate required plugins', 'zn_framework' ); ?></a></p>
			</div>
		<?php
	}
	...
	function checkPluginsDependencies(){
		...
		foreach ( $pluginsRequirements as $slug => $pluginConfig ) {
			if( ! ZNHGTFW()->getComponent( 'addons_manager' )->is_plugin_active( $pluginConfig['slug'] ) ){
				...
				$this->addError( sprintf( esc_html__( 'The %s is a required plugin', 'zn_framework' ), $pluginData['name']) );
			}
		}
