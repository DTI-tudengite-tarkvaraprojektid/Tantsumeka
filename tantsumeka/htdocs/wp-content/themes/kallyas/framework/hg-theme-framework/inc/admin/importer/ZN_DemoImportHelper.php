	...
	public static function getDemoConfig( $demoDirPath )
	{
		...
		if( $fs->is_file($filePath)){
			if( $fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Demo config file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getImagesMap( $demoDirPath )
	{
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Images map file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getGlobalOptions( $demoDirPath )
	{
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Global options file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getWpSettings( $demoDirPath )
	{
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('WP Settings file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getThemeOptions( $demoDirPath )
	{
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Theme options file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getMenus( $demoDirPath )
	{
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Menus file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getPluginsOptions( $demoDirPath )
	{
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Plugins file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getWidgets( $demoDirPath )
	{
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Widgets file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getTerms( $demoDirPath ){
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Terms file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
	...
	public static function getTermsMeta( $demoDirPath ){
		...
		if($fs->is_file($filePath)){
			if($fs->is_readable($filePath)) {
				...
				self::log(sprintf (
					__('Terms meta file is not accessible (%s). ', 'zn_framework'),
					$filePath
				));
				return false;
			}
...
//<editor-fold desc="::: IMPORT FUNCTIONS">
	...
	public static function importCustomFonts( $demoDirPath )
	{
		...
		if(! $extracted){
			self::log( sprintf (
				__('Error extracting the custom fonts from (%s). ', 'zn_framework'), $archivePath )
			);
			return false;
		}
			...
			if( ! empty($demoInfo) )
			{
				...
				foreach( $demoPlugins as $pluginSlug )
				{
					...
					if( $znAddonsMgr->is_plugin_installed( $pluginSlug ) ){
						if( ! $znAddonsMgr->is_plugin_active( $pluginSlug ) ){
...
//							error_log( '[DEMO IMPORTER] Error adding the term: ' . $term['name'] . ': ' . $resultInsert->get_error_message() );
	...
	public static function importDemoImages( $demoDirPath )
	{
		...
		if( ! $extracted ){
			self::log(sprintf (
				__('Error extracting the demo images from (%s). ', 'zn_framework'),
				$imagesArchive
			));
			return false;
		}
