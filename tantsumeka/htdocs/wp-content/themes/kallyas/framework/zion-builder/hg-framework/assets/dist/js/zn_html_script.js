...
(function($)
{
	...
	$.ZnHtml.prototype = {
		...
		addactions : function() {
			...
			// Icon font functionality
			fw.delete_icon_font();
			fw.download_icon_font();
			fw.upload_icon_font();
		...
		smart_select : function(scope){
			...
			element.each(function(index, el) {
				...
				items.hover(function(event) {
					$el.find('.zn-smartselect-hoverImg img').attr('src', $(this).attr('data-img'));
				});
			...
			$('.zn_run_theme_updater').click(function(e) {
				...
				new $.ZnModalConfirm( 'We strongly recommend making a full theme backup ? Please note that data loss may occur. Are you sure you want to continue ?', 'Cancel', 'Perform theme update', function(){
					...
					var data = {
						...
						data:false,
						action: 'znhgkl_process_theme_updater',
						zn_ajax_nonce: ZnAjax.security
					};
		...
		enable_tinymce : function(scope){
			...
			if( length>0 ) {
				for (var i=0; i<length; i++) {
					...
					// Init tinymce
					if( window.tinyMCEPreInit && window.tinyMCEPreInit.mceInit.zn_dummy_editor_id ) {

						// Get an instance of tinyMCE init config
						window.tinyMCEPreInit.mceInit[id] = _.extend( _.clone( window.tinyMCEPreInit.mceInit.zn_dummy_editor_id ), {
							id: id,
							selector: '#' + id
						});
		...
		launch_ui_slider : function( scope ) {
			...
			$(element).each(function() {
				...
				var $el = $(this),
					...
					step = parseFloat($el.attr('data-step')) || 1,
				...
				$el.slider({
					...
					min: $min,
					max: $max,
					value: parseFloat($el.attr('data-value')) || $min,
					step: step,
				...
				$slider_input.on( 'change', function(){
					...
					if( parseFloat($elInputVal) < $min ) { $this.val( $min ); }
					if( parseFloat($elInputVal) > $max ) { $this.val( $max ); }
					...
					if ($elInputVal == 'auto') {
						...
						$el.slider( "value" , parseFloat( $elInputVal ) );
					}
				});
		...
		icon_list : function(scope){
			...
			$(element).hover(function(e){
				...
				if( $('.znIconHolder').length === 0 ){
					$('body').append('<div class="znIconHolder"></div>');
				}

				$('.znIconHolder').show().css({ left:left, top:top}).html($(this).clone());

			},
			function(){
				$('.znIconHolder').hide().html('');
			});
		...
		clone_el : function(scope){
			...
			$(element).click(function(e) {
				...
				// Fix icon dropdown not working
				var iconToggles = new_content.find('.zn_icon_op_label');
				iconToggles.each(function(i, element){
					var newId = $(this).attr('for') + i;
					$(this).attr('for', newId);
					$(this).prev('.zn_icon_op_checkbox').attr('id', newId);
				});
				// Create an event for new content received
				fw.scope.trigger({type: "ZnNewFWContent",content : new_content});
				...
				container.append(new_content).zn_sortable_order();
				// Allow othersto know we have cloned an element
				el.trigger('znpb:element:cloned', [ to_be_cloned, new_content ] );
				// Check if we have a max items set
				if (  typeof max_items !== 'undefined'  ){
					var childrens_num = container.children('.zn_group').length;
					if( childrens_num >= max_items ){
		...
		add_g_font : function(){
			...
			$('.zn_add_gfont').click(function(e) {
				...
				var data = {
					...
					zn_json: JSONdata,
					action: 'znhgfw_html_google_font_add',
					zn_ajax_nonce: nonce
				};
		...
		/**
		 * Will delete an icon font package
		 */
		delete_icon_font : function( scope ){
			var fw = this;

			$(document).on( 'click', '.js-zn-remove-icon-font', function(e){

				var font = $(this).parent(),
					data = {
						action: 'znhg_remove_icon_font',
						font_name: $(this).data('font_name'),
						security: ZnAjax.security
					};

				new $.ZnModalConfirm( 'Are you sure you want to delete this icon package?', 'No', 'Yes, delete the icon package', function(){

					var $wrapper = $(this).closest('.js-znhg_icon_font_container');

					// Don't proceed if an action is already in progress
					if( $wrapper.hasClass('znhg_icon_font_container--loading') ){
						return;
					}

					// Add a loading class
					$wrapper.addClass('znhg_icon_font_container--loading');

					// Make the ajax call
					jQuery.post( ZnAjax.ajaxurl, data, function( response ) {

						// If the font was succesfully deleted
						if ( response.success ) {
							new $.ZnModalMessage( response.data.message );
							font.remove();
						}
							...
							new $.ZnModalMessage('There was a problem downloading the icons !');
						}

						// Remove the loading class
						$wrapper.removeClass('znhg_icon_font_container--loading');
		...
		/**
		 * Will download an icon font package
		 */
		download_icon_font : function(){
			var fw = this;

			$(document).on( 'click', '.js-zn-download-icon-font', function(e){
				...
				var data = {
					action: 'znhg_get_download_icon_font_url',
					font_name: $(this).data('font_name'),
					security: ZnAjax.security
				};
				...
				jQuery.post( ZnAjax.ajaxurl, data, function( response ) {
					...
					// If we got an error
					if ( response.success ) {
						window.location.assign( response.data.archive_url );
					}
					else if( ! response.success ){
						new $.ZnModalMessage( response.data.message );
					}
		...
		upload_icon_font : function(){
			...
			trigger.on( 'click', function(e){
				...
				fw.file_uploader = wp.media.frames.file_frame = wp.media({
					...
					var data = {
						action: 'znhg_upload_icon_package',
					...
					jQuery.post( ZnAjax.ajaxurl, data, function( response ) {
						...
						if ( response.success ) {
							new $.ZnModalMessage( response.data.message );
							$('.uploads_container').append( response.data.html );
						}
						else{
...
/* Link media button */
jQuery('body').on( 'click', '.zn_media_link_button_trigger', function(event){
	var file_frame;
	event.preventDefault();
	var button = jQuery(this),
		title = button.data('title'),
		zn_button = button.data('button'),
		targetFieldUrl = jQuery('#'+button.data('targetUrl')),
		targetTitleField = jQuery('#'+button.data('targetTitle'));
	...
	file_frame.on( 'select', function() {
		...
		// Update references
		targetFieldUrl.val(attachment.url).trigger('change');
		targetTitleField.val(attachment.title).trigger('change');
	});
