...
add_filter( 'post_thumbnail_html', 'zn_woocommerce_post_thumbnail_html', 98, 4 );
...
if ( ! function_exists( 'zn_woocommerce_post_thumbnail_html' ) ) {
	...
	function zn_woocommerce_post_thumbnail_html( $html, $pid, $tid, $size )
	{
		global $product, $woocommerce;

		// Fix WooCommerce 3.3.0 compatibility
		if( ! in_array( $size, array( 'woocommerce_thumbnail', 'shop_catalog' ) ) ||  ! $product ){
			return $html;
		}
		...
		if( ! empty( $woo_prodLoop_image_size ) ){
			...
			$attachment_url = wp_get_attachment_url( $tid );
			$attachment_title = get_the_title($tid);
			...
			$img = sprintf(
				...
				$attachment_title,
				get_post_meta($tid, '_wp_attachment_image_alt', true),
				image_hwstring( $width, $height )
			);
		}
		else {
			$img = wp_get_attachment_image( $tid, 'shop_catalog', false, array('class'=>'kw-prodimage-img') );
		}
			...
			?>
				<div class="kl-addedtocart">
					<div class="kl-addedtocart-container">
						...
						<a href="<?php echo wc_get_checkout_url(); ?>" class="kl-addedtocart-checkout btn btn-lined lined-dark btn-md"><?php _e( 'CHECKOUT &rarr;', 'zn_framework' );?></a>
		...
		if ( zget_option( 'woo_show_cart', 'zn_woocommerce_options' ) ) {
			...
			<ul class="sh-component topnav navLeft topnav--cart topnav-no-sc topnav-no-hdnav">
				<li class="drop topnav-drop topnav-li">
					...
					<a id="mycartbtn" class="kl-cart-button topnav-item kl-cart--<?php echo $cart_style; ?>" href="<?php echo wc_get_cart_url(); ?>" title="<?php _e( 'View your shopping cart', 'zn_framework' ); ?>">
						<?php
							zn_cart_icon_markup();
