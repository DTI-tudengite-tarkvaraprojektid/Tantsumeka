...
function adminBarComingSoonNotice( $wp_admin_bar ) {
		...
		if( is_user_logged_in() && current_user_can( 'administrator' ) && zget_option( 'cs_enable', 'coming_soon_options', false, 'no' ) == 'yes'  ){
			$args = array(
...
if( ! function_exists( 'zn_modify_link_url_for_modals' ) ){
	function zn_modify_link_url_for_modals( $linkArray ){
		...
		if( isset( $linkArray['target'] ) && $linkArray['target'] == 'modal_iframe' ){
			...
			if(
				isset( $linkArray['url'] ) &&
				(
					strpos( $linkArray['url'], $ytshorturl ) !== false ||
					strpos( $linkArray['url'], $ytlongurl ) !== false
				)
			){

				// Convert short link to full youtube link
				if (strpos($linkArray['url'],$ytshorturl) !== false) {
...
if ( !function_exists( 'vt_resize' ) )
{
	...
	function vt_resize( $attach_id = null, $img_url = null, $width = 0, $height = 0, $crop = false )
	{
		...
		if ( is_array( $image ) ) {
			...
			$return['url'] = $image != 'image_not_specified' && $image != 'getimagesize_error_common' && $image != '' ? $image : $img_url;
		}
...
function zn_mailchimp_register()
{
	if ( ( 'POST' == strtoupper($_SERVER['REQUEST_METHOD'])) && isset ( $_POST[ 'zn_mc_email' ] ) )
	{
        ...
        if( ! isset($_POST['zn_hg_mailchimp']) || ! wp_verify_nonce( $_POST['zn_hg_mailchimp'], 'zn_hg_mailchimp')) {
			...
			$msg .= '<button type="button" class="close" data-dismiss="alert" aria-label="'.esc_html( __( 'Close', 'zn_framework') ).'">';
			$msg .= '<span aria-hidden="true">&times;</span></button><strong>' . __( 'Error:', 'zn_framework' ) . '</strong>&nbsp; ';
			$msg .= __( 'Error: Invalid nonce.', 'zn_framework' );
			$msg .= '</div>';
			exit( $msg );
        }
		...
		if ( empty ( $mcApiKey ) ) {
			...
			$msg .= '<button type="button" class="close" data-dismiss="alert" aria-label="'.esc_html( __( 'Close', 'zn_framework') ).'">';
			$msg .= '<span aria-hidden="true">&times;</span></button><strong>' . __( 'Error:', 'zn_framework' ) . '</strong>&nbsp; ';
			$msg .= __('Please add the API key in the theme options.', 'zn_framework');
			$msg .= '</div>';
			exit( $msg );
        }
		else
		{
			...
			$merge_vars = array(
				'EMAIL' => esc_attr($_POST[ 'zn_mc_email' ])
			);

			if ( $mcapi->subscribe( esc_attr($_POST[ 'zn_mailchimp_list' ]), esc_attr($_POST[ 'zn_mc_email' ]), $merge_vars ) )
			{
				$text_msg = '';
				if( $double_opt_in === 'yes' ){
					$text_msg = __( 'Success!&nbsp; Check your inbox or spam folder for a message containing a confirmation link.', 'hogash-mailchimp' );
				}
				else{
					$text_msg = __( 'Success!&nbsp; You have successfully signed up.', 'hogash-mailchimp' );
				}

				$msg = '<div class="alert alert-success alert-dismissible" role="alert">
<button type="button" class="close" data-dismiss="alert" aria-label="'.esc_html( __( 'Close', 'zn_framework') ).'">
<span aria-hidden="true">&times;</span></button>' . $text_msg . '</div>';
			}
			else {
				$msg = '<div class="alert alert-danger alert-dismissible" role="alert">
<button type="button" class="close" data-dismiss="alert" aria-label="'.esc_html( __( 'Close', 'zn_framework') ).'">
<span aria-hidden="true">&times;</span></button><strong>' . __( 'Error:', 'zn_framework' ) . '</strong>&nbsp; ' . $mcapi->getErrorMessage() . '</div>';
			}
			exit( $msg );
		}
	}
